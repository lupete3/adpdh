...
<main id="contenu">
<div class="container breadcrumb" aria-label="Fil d’Ariane"><a href="{{ route('admin.cms.preview', $previewPages['index']) }}">Accueil</a><span aria-hidden="true">/</span><a href="{{ route('admin.cms.preview', $previewPages['actualites']) }}">Actualités</a><span aria-hidden="true">/</span><span>Article de démonstration</span></div><section class="container about-heading"><p class="eyebrow">{{ $titles['text-6'] }}</p><h1>{{ $titles['text-3'] }}</h1><p>Formation</p></section><div class="container collection-notice"><span class="demo-label">Exemple fictif — à remplacer</span><p>Aucun événement réel n’est décrit. Date de publication : à renseigner lors du remplacement par un article validé.</p></div><div class="container article-cover"><figure class="editorial-photo "><img class="" src="/adpdh/assets/workshop-1200.webp" srcset="/adpdh/assets/workshop-640.webp 640w, /adpdh/assets/workshop-1200.webp 1200w, /adpdh/assets/workshop-1680.webp 1680w" sizes="(max-width: 640px) 100vw, 75vw" width="1680" height="1260" alt="Illustration : des entrepreneures dans un atelier de couture" loading="lazy" decoding="async"><figcaption>Image d’illustration générée par IA · à remplacer</figcaption></figure></div><article class="container article-body"><p class="lead">Cet exemple illustre le futur récit d’une séance consacrée à la gestion d’une activité génératrice de revenus.</p><h2>{{ $titles['text-4'] }}</h2><p>Dans ce scénario fictif, les participants découvrent la tenue d’une caisse, la distinction entre dépenses professionnelles et dépenses du foyer, ainsi que la préparation des achats.</p><h2>{{ $titles['text-5'] }}</h2><p>La publication finale pourra présenter les objectifs de la séance, les exercices réellement réalisés et les observations validées par l’équipe. Aucun effectif ni résultat de formation n’est attribué à cet exemple.</p><aside class="collection-notice"><strong>Pour découvrir les activités réelles</strong><p>La fiche AVEC & AGR présente les informations communiquées par ADPDH sur son accompagnement.</p><a class="text-link" href="{{ route('admin.cms.preview', $previewPages['activite-avec-agr']) }}">Découvrir AVEC & AGR →</a></aside></article><div class="container activity-back"><a class="text-link" href="{{ route('admin.cms.preview', $previewPages['actualites']) }}">← Toutes les actualités</a></div>
</main>
