...
<header class="header">
  <div class="container navigation">
    ...
    <nav id="main-nav" aria-label="Navigation principale">
      ...
      <a href="{{ route('admin.cms.preview', $previewPages['temoignages']) }}" aria-current="page">Nos succès</a>
...
<main id="contenu">
<div class="container breadcrumb" aria-label="Fil d’Ariane"><a href="{{ route('admin.cms.preview', $previewPages['index']) }}">Accueil</a><span aria-hidden="true">/</span><span>Nos succès / Témoignages</span></div><section class="container about-heading"><p class="eyebrow">{{ $titles['text-7'] }}</p><h1>{{ $titles['text-3'] }}<br><em>{{ $titles['text-4'] }}</em></h1><p>Les récits donnent une place aux expériences individuelles, aux changements du quotidien et aux perspectives des personnes accompagnées.</p></section><div class="container collection-notice"><strong>Premiers témoignages en cours de collecte</strong><p>Aucun entretien réel n’est publié pour le moment. L’exemple ci-dessous présente le futur format des récits.</p></div><section class="section container"><article class="testimony-card"><div class="portrait-placeholder" role="img" aria-label="Emplacement de portrait : aucun bénéficiaire réel représenté"><span aria-hidden="true">ADPDH<span class="portrait-circle"></span></span><p>Portrait à venir<br><small>Aucun bénéficiaire réel représenté</small></p></div><div class="testimony-copy"><span class="demo-label">Exemple fictif — à remplacer</span><p class="eyebrow">{{ $titles['text-8'] }}</p><h2>{{ $titles['text-5'] }}</h2><blockquote>« Je distingue mieux les besoins de mon activité de ceux de mon foyer. Cette organisation m’aide à préparer les prochaines étapes. »</blockquote><p>Personnage de démonstration. Cette citation n’a pas été recueillie auprès d’une personne accompagnée par ADPDH.</p><a class="button" href="{{ route('admin.cms.preview', $previewPages['temoignage-exemple']) }}">Lire le récit d’exemple →</a></div></article></section><section class="container cross-action"><p class="eyebrow">{{ $titles['text-9'] }}</p><h2>{{ $titles['text-6'] }}</h2><p>La page Impact rassemble les chiffres communiqués par ADPDH, avec leurs périodes et leurs limites.</p><a class="text-link" href="{{ route('admin.cms.preview', $previewPages['impact']) }}">Découvrir notre impact →</a></section>
</main>
