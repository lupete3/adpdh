...
@forelse($activities as $activity)<tr><th>{{ $activity->title }}</th><td>{{ $activity->publication_state === 'published' ? ($activity->published_at?->isFuture() ? 'Programmée' : 'Publiée') : 'Brouillon' }}@if($activity->is_demo) · Démonstration @endif</td><td>{{ ($activity->published_at ?? $activity->created_at)->format('d/m/Y') }}</td><td><a class="btn btn-outline-primary btn-sm" href="{{ route('admin.cms.activities.edit', $activity) }}">Modifier</a></td></tr>@empty<tr><td colspan="4">Aucune activité. Ajoutez votre premier article.</td></tr>@endforelse
</tbody></table></div>
