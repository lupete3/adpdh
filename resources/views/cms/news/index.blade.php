...
@forelse($posts as $post)<tr><th>{{ $post->title }}</th><td>{{ $post->category }}</td><td>{{ $post->is_demo ? 'Démonstration (masquée)' : ($post->status === 'published' ? ($post->published_at?->isFuture() ? 'Programmée' : ($post->published_at ? 'Publiée' : 'Sans date (masquée)')) : 'Brouillon') }}</td><td>{{ $post->published_at?->format('d/m/Y H:i') ?? '—' }}</td><td><a class="btn btn-outline-primary btn-sm" href="{{ route('admin.cms.news.edit', $post) }}">Modifier</a></td></tr>@empty<tr><td colspan="5">Aucune actualité. Ajoutez votre premier article.</td></tr>@endforelse
</tbody></table></div>
